...
@interface ImLinesView(){

}
@end

@implementation ImLinesView
...
-(void)drawRect:(CGRect)rect
{
    ...
    if (self.isK) {
        ...
        for (NSArray *item in self.points) {
            ...
            heightPoint = [[item objectAtIndex:0] CGPointValue];
            lowPoint = [[item objectAtIndex:1] CGPointValue];
            openPoint =[[item objectAtIndex:2] CGPointValue];
...
-(void)drawLineWithContext:(CGContextRef)context
{
    ...
    if (self.startPoint.x == self.endPoint.x && self.endPoint.y == self.startPoint.y)
    {
        ...
        for (id item in self.points)
        {
            ...
            if (currentPoint.x >= 0
                && currentPoint.y >= 0
                && currentPoint.y <= self.frame.size.height
                && currentPoint.x <= self.frame.size.width)
            {
                if (item == [self.points firstObject])
                {
                  ...
                  width:(CGFloat)width
{
    ...
    BOOL isKong = NO;
    ColorModel *colormodel = [UIColor RGBWithHexString:@"#cd2627" withAlpha:self.alpha]; // 设置默认红色
    // 如果开盘价坐标在收盘价坐标上方 则为绿色 即空
    if (openPoint.y < closePoint.y) {
        isKong = YES;
        colormodel = [UIColor RGBWithHexString:@"#05e47a" withAlpha:self.alpha]; // 设置为绿色
    }
    // 设置颜色
    CGContextSetRGBStrokeColor(context, (CGFloat)colormodel.R/255.0f, (CGFloat)colormodel.G/255.0f, (CGFloat)colormodel.B/255.0f, self.alpha);
    // 首先画一个垂直的线包含上影线和下影线
