...
- (instancetype)initWithTitle:(NSString *)title titleColor:(UIColor *)titleColor messageAttributedStr:(NSAttributedString *)message {
    self = [super init];
    if (self) {
...
- (instancetype)initWithTitle:(NSString *)title messageAttributedStr:(NSAttributedString *)message {
    return [self initWithTitle:title titleColor:[UIColor blackColor] messageAttributedStr:message];
    
}
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message {
    return [self initWithTitle:title titleColor:[UIColor blackColor] messageAttributedStr:[[NSAttributedString alloc] initWithString:message]];
}
...
-(void)buildViews{
    ...
    //message
    CGFloat messageHeigh = [self heightWithString:_message.string fontSize:14 width:AlertWidth-2*AlertPadding];
    ...
    [_labelmessage setAttributedText:_message];
...
- (void)addButton:(ButtonType)type withTitle:(NSString *)title buttonBackgroundColor:(UIColor *)bgColor handler:(JKAlertDialogHandler)handler{
    JKAlertDialogItem *item = [[JKAlertDialogItem alloc] init];
    item.title = title;
    item.action = handler;
    item.type = type;
    item.buttonBgColor = bgColor;
    [_items addObject:item];
    item.tag = [_items indexOfObject:item];
}
- (void)addButtonItem {
...
//        button.translatesAutoresizingMaskIntoConstraints = NO;
        button.frame = CGRectMake(idx*width, 0, width, MenuHeight);
        //seperator
        button.backgroundColor = item.buttonBgColor;
...
//        button.layer.borderColor = [[UIColor lightGrayColor] CGColor];
        ...
        if (idx != _items.count - 1) {
            UILabel *lineLabel = [[UILabel alloc] initWithFrame:CGRectMake(width - 0.5 , 0, 0.2, _buttonScrollView.frame.size.height)];
    ...
    UILabel *lineLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, _buttonScrollView.frame.size.width, 0.4)];
    [lineLabel setBackgroundColor:[UIColor lightGrayColor]];
    [_buttonScrollView addSubview:lineLabel];
