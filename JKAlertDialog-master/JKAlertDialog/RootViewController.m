...
- (IBAction)normalTouched:(id)sender {
    ...
    NSString *str = @"这是一个AlertDialog\n";
    NSMutableAttributedString *attriString = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@%@%@%@%@",str,str,str,str,str]];
    [attriString setAttributes:@{NSForegroundColorAttributeName:[UIColor redColor]} range:NSMakeRange(str.length*2, str.length)];
     
     
    JKAlertDialog *alert = [[JKAlertDialog alloc]initWithTitle:@"这是一个AlertDialog" titleColor:[UIColor blueColor] messageAttributedStr:attriString];

    [alert addButton:Button_OTHER withTitle:@"取消" handler:^(JKAlertDialogItem *item) {
        NSLog(@"click %@",item.title);
        
    }];;
    [alert addButton:Button_OK withTitle:@"确定" buttonBackgroundColor:[UIColor redColor] handler:^(JKAlertDialogItem *item) {
        NSLog(@"click %@",item.title);
    }];
